...
export default function HomePage() {
  ...
  const heroStats = useMemo(() => {
    ...
    const actualizacionesHoy = projects.filter(
      (project) => new Date(project.lastUpdate).toDateString() === today
    ).length;
  ...
  const allTranches = useMemo(() => {
    const trancheSet = new Set(projects.map((p) => p.currentTramo).filter(Boolean));
    return Array.from(trancheSet)
      .sort()
      .map((t) => ({ value: t, label: t }));
  }, [projects]);

  const allStatuses = useMemo(() => {
    return [...new Set(projects.map((project) => project.projectStatus))];
  }, [projects]);
  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects.filter((project) => {
      ...
      const matchesTranche =
        ...
        selectedTranches.includes(project.currentTramo);
  ...
  function toggleTranche(tranche) {
    setSelectedTranches((prev) =>
      prev.includes(tranche) ? prev.filter((t) => t !== tranche) : [...prev, tranche]
    );
  }
