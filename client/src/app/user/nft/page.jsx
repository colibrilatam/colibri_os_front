...
export default function NftPage() {
  ...
  const {
    execute: getNftProjectsInfo,
    error: nftProjectsInfoError,
    loading: nftProjectsInfoLoading,
  } = useRequest(nftService.getNftProjects);
  const {
    execute: getTramo,
    error: tramosError,
    loading: tramosLoading,
  } = useRequest(projectsService.currentTramo);
  const {
    execute: getUserData,
    error: userDataError,
    loading: userDataLoading,
  } = useRequest(userService.userData);

  async function getData() {
    setLoading(true);
    try {
      const { data: allProjects, error: allProjectsError } =
        await getNftProjectsInfo();
      ...
      if (allProjects) {
        ...
        const { data: FCUserData } = await getUserData(
          flujoClave.project.ownerUserId,
        );
        const { data: FCTramoData } = await getTramo(
          flujoClave.project.currentTramoId,
        );
        const ic = getProjectIC(flujoClave.project.projectName);

        allProjectsInfo.push({
          nftProject: flujoClave,
          user: FCUserData,
          tramo: FCTramoData,
          ic: ic,
        });
      ...
      for (let i = 0; i < 2; i++) {
        const { data: userData } = await getUserData(
          allProjects[i].project.ownerUserId,
        );
        const { data: tramoData } = await getTramo(
          allProjects[i].project.currentTramoId,
        );
        const ic = getProjectIC(allProjects[i].project.projectName);

        allProjectsInfo.push({
          nftProject: allProjects[i],
          user: userData,
          tramo: tramoData,
          ic: ic,
        });
    ...
    <div className="z-0 relative flex flex-col lg:flex-row w-full h-full p-1 lg:p-3 gap-2">
      <div className="glass-effect border-glass rounded-2xl p-6 min-w-1/3">
        ...
        <div className="flex flex-col  md:justify-between gap-2">
          ...
          <p className="w-fit primary-button border-glass rounded-full p-2 text-white">
            {t('portafolioNft')}
          </p>
        </div>
        <div className="w-full items-center flex justify-center">
          <img
            src={svg.src}
            alt="NFT Avatar"
            className="p-2 glass-effect-dark w-80 h-80 rounded-full"
          ></img>
        </div>
      ...
      <div className=" p-4 w-full rounded-2xl glass-effect border-glass">
        <h1 className="font-bold mb-6 text-(--text-primary)">
          {t('portafolioBecados')}
        </h1>
        <div className="w-full overflow-x-auto rounded-2xl glass-effect border-glass">
          <table className="w-full text-sm">
            ...
            <tbody>
              {projectsInfo &&
                projectsInfo.length === 3 &&
                projectsInfo.map((project, index) => (
                  <tr
                    key={project.nftProject.id}
                    className={`border-b  border-gray-100 last:border-0 ${
                      index % 2 === 0
                        ? 'bg-var(--theme-surface-background)'
                        : 'glass-effect-dark'
                    }`}
                  >
                    {/* Colibri / Proyecto */}
                    <td className="px-4 py-3">
                      <p className="font-semibold text-(--text-primary)">
                        {project.nftProject.project.projectName}
                      </p>
                      <p className="text-xs text-(--text-secondary)">
                        {project.nftProject.project.industry}
                      </p>
                      <p className="text-xs text-(--text-tertiary)">
                        {project.nftProject.project.country}
                      </p>
                    </td>
